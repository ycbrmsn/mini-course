...
--[[
  获得两个二维向量的夹角
  @param  {number | YcVector2 | table} x1 数值或二维向量对象或二维向量表
  @param  {number | YcVector2 | table} y1 数值或二维向量对象或二维向量表
...
function YcVectorHelper.getTwoVector2Angle (x1, y1, x2, y2)
  ...
  local msg = YcStringHelper.concat('获取二维向量夹角参数错误：x1=', x1, ',y1=', y1, ',x2=', x2, ',y2=', y2)
  error(msg)
end
...
function YcVectorHelper.getTwoVector3Angle (x1, y1, z1, x2, y2, z2)
  ...
  local msg = YcStringHelper.concat('获取三维向量夹角参数错误：x1=', x1, ',y1=', y1, ',z1=', z1, ',x2=', x2, ',y2=', y2, ',z2=', z2)
  error(msg)
end
...
--[[ 
  获取竖直旋转角度。使水平方向上的两个分量对应相同，就可以保证两向量在同一个竖直平面上
  然后取到的夹角就是竖直方向上的夹角
  @param  {table} vec3 朝向
  @return {number} 角度
]]--
...
--[[
  ...
  @param  {table} srcPos 起点位置
  @param  {table} dstPos 终点位置
  @param  {number} value 向量大小，默认为1
  @return {YcVector3} 向量
]]
