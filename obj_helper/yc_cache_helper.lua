...
--- 获取对象的位置信息。如果有缓存，则从缓存中取。无论是否使用缓存，都会缓存本次结果
...
--- 获取对象的队伍信息。如果使用缓存，则优先从缓存中取。无论是否使用缓存，都会缓存本次结果
---@param objid integer 对象id
---@param notUseCache boolean | nil 是否不使用缓存，默认使用缓存
...
--- 记录信息一段时间
---@param info table 信息集合
---@param key string | number 键
---@param value any 值
---@param time number 记录保留时长
---@param t string 类型
---@return nil
...
--- 移除对象，并清除缓存。如果该对象缓存了位置，则手动移除时需要使用此方法。用以防止对象已消失，但缓存位置数据还在。
---@param objid integer 对象id
---@return boolean 是否成功
