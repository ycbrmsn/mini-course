...
---@param itemid1 integer 切换前道具id
---@param itemid2 integer 切换后道具id
...
--- 使用道具技能
---@param objid integer 迷你号/生物id
---@param itemid integer 道具id。不传则获取玩家手持道具id
---@param index integer 技能序号，从1开始
---@return nil
...
--- 手持道具点击方块
---@param objid integer 迷你号/生物id
---@param itemid integer 道具类型id。不传则获取玩家手持道具id
...
--- 手持道具攻击命中
---@param objid integer 迷你号/生物id
---@param toobjid integer 命中玩家迷你号/生物id
---@param itemid integer 道具类型id。不传则获取玩家手持道具id
---@return nil
...
---@param toobjid integer 被击中的玩家的迷你号/生物id
---@param blockid integer 被击中的方块类型id
---@param x number 事件发生的位置x
---@param y number 事件发生的位置y
---@param z number 事件发生的位置z
---@return nil
...
--- 投掷物被创建
---@param objid integer 投掷物所属者的迷你号/生物id
---@param projectileid integer 投掷物id
---@param itemid integer 投掷物的道具类型id
---@param x number 事件发生的位置x
---@param y number 事件发生的位置y
---@param z number 事件发生的位置z
---@return nil
...
--- 获取自定义道具投掷物信息
---@param projectileid integer 投掷物id
---@return table | nil 道具投掷物信息，nil表示道具投掷物信息不存在
...
---@param projectileid integer 投掷物id
---@param objid integer 归属者id。表示是谁的投掷物
---@param item YcItem 道具对象
---@param o table 其他需要记录的信息
---@return nil
...
function YcItemHelper.recordMissile(projectileid, attr, val)
  ...
  if YcItemHelper.missile[projectileid] then -- 已存在
    ...
    YcItemHelper.missile[projectileid] = {
      [attr] = val
    }
  end
  -- 保留的记录30秒后删除
...
---@param objid integer 迷你号/生物id
---@param itemid integer 道具id
---@param cd number 冷却时长（秒）
---@param dontSetCD boolean | nil 是否不设置CD。默认要设置CD
---@return nil
...
---@param objid integer 迷你号/生物id
---@param skillname string 技能名称
---@param callback function 取消操作成功后的回调
---@return nil
...
-- 新增道具事件
ScriptSupportEvent:registerEvent([=[Player.AddItem]=], function(event)
...
-- 使用道具事件
ScriptSupportEvent:registerEvent([=[Player.UseItem]=], function(event)
  local objid = event.eventobjid
  local itemid = event.itemid
  local itemnum = event.itemnum
...
-- 消耗道具事件
ScriptSupportEvent:registerEvent([=[Player.ConsumeItem]=], function(event)
  local objid = event.eventobjid
  local itemid = event.itemid
  local itemnum = event.itemnum
...
-- 丢弃道具事件
ScriptSupportEvent:registerEvent([=[Player.DiscardItem]=], function(event)
  local objid = event.eventobjid
  local itemid = event.itemid
  local itemnum = event.itemnum
...
-- 玩家选择快捷键事件
ScriptSupportEvent:registerEvent([=[Player.SelectShortcut]=], function(event)
  local objid = event.eventobjid
  local itemid = event.itemid
  local itemnum = event.itemnum
...
-- 玩家攻击命中事件
ScriptSupportEvent:registerEvent([=[Player.AttackHit]=], function(event)
  local objid = event.eventobjid
...
-- 投掷物击中事件
ScriptSupportEvent:registerEvent([=[Actor.Projectile.Hit]=], function(event)
  local objid = event.eventobjid
  local toobjid = event.toobjid
  local blockid = event.blockid
  local x, y, z = event.x, event.y, event.z
...
-- 投掷物被创建事件
ScriptSupportEvent:registerEvent([=[Missile.Create]=], function(event)
  local objid = event.eventobjid
  local toobjid = event.toobjid
  local itemid = event.itemid
  local x, y, z = event.x, event.y, event.z
